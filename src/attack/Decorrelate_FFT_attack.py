...
class Decorrelate_FFT_attack():
    ...
    def attack_param(self, original_data, decorrelate=True, fft=True):
        ...
        def inner():
            ...
            original_data_input = torch.tensor(np.transpose(original_data, [2, 1, 0])).float().to(self.device)
            return torch.stack([attack_input + original_data_input + self.epsilon/2 * torch.randn_like(original_data_input), original_data_input])
        return params, inner
